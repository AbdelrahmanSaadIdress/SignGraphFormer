"""Full SLR model — Experiment 2: KeypointEncoder + Transformer + ClassificationHead.

SLRModelTransformer is the second experiment. It is structurally identical
to SLRModelLSTM — same forward signature, same checkpoint format, same
sub-module names — so the trainer and evaluator are model-agnostic.
...
from models.temporal_encoder_transformer import TemporalEncoder as TemporalEncoderTransformer

logger = logging.getLogger(__name__)


class SLRModelTransformer(nn.Module):
    """End-to-end Sign Language Recognition model with Transformer temporal encoder.

    Pipeline:
        1. Reshape  (B, T, 225)       → (B*T, 75, 3)
        2. KeypointEncoder            → (B*T, hidden_dim)
        3. Reshape  (B*T, hidden_dim) → (B, T, hidden_dim)
        4. TemporalEncoderTransformer → (B, hidden_dim)
    ...
    def __init__(self, cfg: ModelConfig) -> None:
        ...
        self.temporal_encoder = TemporalEncoderTransformer(cfg)
        self.classification_head = ClassificationHead(cfg)

        num_params = sum(p.numel() for p in self.parameters() if p.requires_grad)
        logger.info("SLRModelTransformer initialized | trainable params: %s", f"{num_params:,}")
    ...
    @classmethod
    def load_checkpoint(cls, path: str, device: str = "cpu") -> "SLRModelTransformer":
        ...
        Returns:
            SLRModelTransformer instance in eval mode.
        """
